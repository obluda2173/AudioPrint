...
- *Module 1: dso.jl* (the ears)

  *Responsibility*: Convert the .mp3 files to 2D array (spectogram)
...
- *Module 2: fingerprint.jl* (the brain)
  ...
  *Logic*:
  1. *Find Peaks*: Look at the 2D spectograms. Find the points that are louder than their neighbours (local maxima).
...
- *What is a spectrogram?*
  ...
  A spectogram is a visual graph showing a signal's frequency changes over time, mapping *time* (horizontal axis), *frequency* (vertical axis), and *amplitude/intensity* (color/brightness) all in one image, allowing you to /"see"/ sound or other signals by revealing how their frequency content evolves.
